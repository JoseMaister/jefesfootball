...
if (!class_exists('\Tablesome\Includes\Modules\TablesomeDB_Rest_Api\TablesomeDB_Rest_Api')) {
    class TablesomeDB_Rest_Api
    {
        ...
        public function save_table($request)
        {
        ...
        public function run_crud($params)
        {
            ...
            if (isset($params['records_deleted']) && is_array($params['records_deleted']) && !empty($params['records_deleted'])) {
                $this->datatable->records->delete_records($params, $params['records_deleted']);
            }

            /** Insert all records  */
            $inserted_records_count = 0;
            if (isset($params['records_inserted']) && !empty($params['records_inserted']) && is_array($params['records_inserted'])) {
                $insert_info = $this->datatable->records->insert_many($params['table_id'], $params['meta_data'], $params['records_inserted']);
            ...
            /**  */
            $response_data = $this->datatable->records->update_records($params);
